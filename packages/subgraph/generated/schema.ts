...
export class FinalConfirmation extends Entity {
  constructor(id: Bytes) {
    super();
    this.set("id", Value.fromBytes(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save FinalConfirmation entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type FinalConfirmation must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("FinalConfirmation", id.toBytes().toHexString(), this);
    }
  }

  static loadInBlock(id: Bytes): FinalConfirmation | null {
    return changetype<FinalConfirmation | null>(
      store.get_in_block("FinalConfirmation", id.toHexString())
    );
  }

  static load(id: Bytes): FinalConfirmation | null {
    return changetype<FinalConfirmation | null>(
      store.get("FinalConfirmation", id.toHexString())
    );
  }
